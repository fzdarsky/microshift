...
build_image() {
    ...
    if [ -n "$parent_version" ]; then
        ...
        buildid=$(sudo composer-cli compose start-ostree --ref rhel/${OS_VERSION_MAJOR}/${BUILD_ARCH}/edge --url http://localhost:8080/repo/ ${blueprint} ${image_type} | awk '{print $2}')
    else
        title "Building ${image_type} for ${blueprint} v${version}"
        buildid=$(sudo composer-cli compose start-ostree --ref rhel/${OS_VERSION_MAJOR}/${BUILD_ARCH}/edge ${blueprint} ${image_type} | awk '{print $2}')
    fi
...
reposync -n -a ${BUILD_ARCH} -a noarch --download-path openshift-local \
            --repo=rhocp-4.11for-rhel-${OS_VERSION_MAJOR}-${BUILD_ARCH}-rpms \
            --repo=fast-datapath-for-rhel-${OS_VERSION_MAJOR}-${BUILD_ARCH}-rpms >/dev/null
...
if [ $(find openshift-local -name '*.rpm' | wc -l) -eq 0 ] ; then
    echo "No RPM packages were found at the 'rhocp-4.11-for-rhel-${OS_VERSION_MAJOR}-${BUILD_ARCH}-rpms' repository. Exiting..."
    exit 1
fi
